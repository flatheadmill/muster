...
function codex_popup_terminal_color {
    typeset setting
...
function :execute:codex:popup {
    ...
    if codex_popup_terminal_color MUSTER_TERMINAL_FOREGROUND; then
        foreground=$REPLY
    fi
    if codex_popup_terminal_color MUSTER_TERMINAL_BACKGROUND; then
        background=$REPLY
    fi
