...
The operation is deliberately cold. Close Puzzle, stop the window's Codex TUI, and stop every private participant popup before renaming. Muster also refuses a rename while an affected Codex thread is active, a nudge is queued, or a Wicket background job may still be active. Idle Codex threads are resumed under the new directory and routing configuration after the durable rename. Renaming a tmux session cannot change the environment or working directory already held by its processes.
